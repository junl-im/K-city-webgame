...
import './styles/alpha133.css';
import './styles/alpha134.css';
...
import { installMobileRuntime112, syncMobileRuntime112, inspectMobileRuntime112 } from './ui/mobileRuntime112';
import { installFieldHud112, syncFieldHud112, inspectFieldHud112 } from './ui/fieldHud112';
import { installMaintenance113, syncMaintenance113, inspectMaintenance113 } from './ui/maintenance113';
import { installStyleLoader115, syncStyleLoader115, inspectStyleLoader115 } from './ui/styleLoader115';
import { installAssetDelivery115, syncAssetDelivery115, inspectAssetDelivery115 } from './ui/assetDelivery115';
import { installFieldLayout116, syncFieldLayout116, inspectFieldLayout116 } from './ui/fieldLayout116';
...
import { installRuntimePolish126, inspectRuntimePolish126, syncRuntimeRoute126 } from './ui/runtimePolish126';
...
import { installResiliencePlan133, inspectResiliencePlan133, syncResilienceRoute133 } from './ui/resiliencePlan133';
import { installReleaseControl134, inspectReleaseControl134, syncReleaseRoute134 } from './ui/releaseControl134';
...
let fullLegacyStylesRequested121 = false;
function loadRuntimeStyles119() {
  if (runtimeStylesRequested119) return;
  runtimeStylesRequested119 = true;
  document.body.classList.add('runtime-style-loading-121');
  // 1.21: 첫 START 직후 800KB급 legacy styles.css를 다시 받지 않는다.
  // 요청 UI에 필요한 소형 보정 CSS만 지연 로드하고, 전체 구형 CSS는 수동 옵션에서만 허용한다.
  void import('./styles/alpha116.css').then(() => {
    document.body.classList.remove('runtime-style-loading-119', 'runtime-style-loading-121');
    document.body.classList.add('runtime-style-ready-119', 'runtime-style-ready-121');
  }).catch((error) => {
    document.body.classList.remove('runtime-style-loading-119', 'runtime-style-loading-121');
    document.body.classList.add('runtime-style-failed-119', 'runtime-style-failed-121');
    console.warn('[SoulOnline 1.21] compact runtime CSS load skipped', error);
  });
}

function loadFullLegacyStyles121(reason = 'manual') {
  if (fullLegacyStylesRequested121) return;
  const allowed = new URLSearchParams(window.location.search).get('legacyCss') === '1'
    || localStorage.getItem('soul-online-allow-legacy-css') === '1';
  if (!allowed) return;
  fullLegacyStylesRequested121 = true;
  document.body.classList.add('legacy-style-loading-121');
  void import('./styles.css').then(() => {
    document.body.classList.remove('legacy-style-loading-121');
    document.body.classList.add('legacy-style-ready-121');
    console.info(`[SoulOnline 1.21] legacy CSS enabled: ${reason}`);
  }).catch((error) => {
    document.body.classList.remove('legacy-style-loading-121');
    document.body.classList.add('legacy-style-failed-121');
    console.warn('[SoulOnline 1.21] legacy CSS load skipped', error);
  });
}
...
async function boot() {
  document.body.classList.add('fantasy-ui-119', 'fantasy-ui-120', 'fantasy-ui-121', 'fantasy-ui-122', 'fantasy-ui-124', 'fantasy-ui-125', 'fantasy-ui-126', 'fantasy-ui-127', 'fantasy-ui-128', 'emergency-boot-119', 'recovery-kernel-120', 'polish-kernel-121', 'stability-kernel-122', 'field-load-polish-124', 'field-runtime-polish-125', 'runtime-polish-126', 'title-revival-127', 'dependency-audit-128', 'fantasy-ui-129', 'connection-integrity-129', 'visual-quality-preserved-129', 'fantasy-ui-130', 'login-connection-130', 'visual-quality-preserved-130', 'firebase-free-plan-131', 'visual-quality-preserved-131', 'fantasy-ui-133', 'resilience-plan-133', 'visual-quality-preserved-133', 'firebase-freeplan-safe-133', 'fantasy-ui-134', 'release-control-134', 'visual-quality-preserved-134', 'firebase-freeplan-safe-134', 'fantasy-ui-135', 'visual-asset-kit-135', 'visual-quality-preserved-135', 'fantasy-ui-136', 'visual-asset-kit-136', 'visual-quality-preserved-136', 'boot-critical-119', 'standard-mode-119', 'title-layout-116', 'no-pet-116');
  ...
  installRuntimePolish126(document, { appShell: document.querySelector<HTMLElement>('#app'), gameRoot: root, titleScreen, loginScreen, townScreen, startButton: startGameBtn });
  ...
  installResiliencePlan133(document, { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  installReleaseControl134(document, { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  installVisualAssetKit135(document, { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  installVisualAssetKit136(document, { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  ...
  // 1.20 복구 원칙: 버튼/로그인 바인딩을 네트워크·Firebase보다 먼저 끝낸다.
  const prestartedLogin120 = Boolean((window as Window & { SOUL_PRESTART_ROUTE_120?: string }).SOUL_PRESTART_ROUTE_120 === 'login' || document.body.classList.contains('prestart-login-120'));
  if (prestartedLogin120) {
    ...
    syncResilienceRoute133(document, 'login', { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncReleaseRoute134(document, 'login', { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncVisualAssetRoute135(document, 'login', { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncVisualAssetRoute136(document, 'login', { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  } else {
    ...
    syncRuntimeRoute126(document, 'title', { appShell: document.querySelector<HTMLElement>('#app'), gameRoot: root, titleScreen, loginScreen, townScreen, startButton: startGameBtn });
    ...
    syncResilienceRoute133(document, 'title', { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncReleaseRoute134(document, 'title', { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncVisualAssetRoute135(document, 'title', { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncVisualAssetRoute136(document, 'title', { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  }
  ...
  // 로컬 저장은 즉시 읽고, Firebase/Auth는 뒤에서 붙인다. 이 줄이 START를 막으면 안 된다.
  ...
  void saveService.init().then(() => {
    ...
    renderCharacterSlots();
    updateWorldSummary();
  }).catch((error) => console.warn('[Firebase] background init skipped', error));
...
function scheduleDeferredBootStack119() {
  ...
  // 1.20: 구형 0.93~1.18 보정 스택은 첫 실행을 흔들 수 있어 기본 비활성화한다.
  // 필요한 최소 CSS와 팝업 모션만 idle 시점에 붙인다.
  runIdleBatch119('recovery-boot-stack-120', [
...
function syncLegacyVisualStack114() {
  ...
  syncRuntimeRoute126(document, route, { appShell: document.querySelector<HTMLElement>('#app'), gameRoot: root, titleScreen, loginScreen, townScreen, startButton: startGameBtn });
  ...
  syncResilienceRoute133(document, route, { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  syncReleaseRoute134(document, route, { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
...
async function enterTown(save: PlayerSave, label = '마을로 이동 중') {
  await withSceneTransition(label, async () => {
    ...
    syncRuntimeRoute126(document, 'town', { appShell: document.querySelector<HTMLElement>('#app'), gameRoot: root, titleScreen, loginScreen, townScreen, startButton: startGameBtn });
    ...
    syncReleaseRoute134(document, 'town', { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncVisualAssetRoute135(document, 'town', { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncVisualAssetRoute136(document, 'town', { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncLegacyVisualStack114();
...
async function startField(save: PlayerSave, zoneId = 'slime-forest', autoStart = false) {
  ...
  try {
    ...
    syncRuntimeRoute126(document, 'field', { appShell: document.querySelector<HTMLElement>('#app'), gameRoot: root, titleScreen, loginScreen, townScreen, startButton: startGameBtn });
    ...
    syncReleaseRoute134(document, 'field', { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncVisualAssetRoute135(document, 'field', { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    syncVisualAssetRoute136(document, 'field', { appShell: document.querySelector<HTMLElement>('#app'), titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
      syncLegacyVisualStack114();
...
async function ensureFullscreen(forceToast = false) {
  // Alpha 1.17: 모바일 브라우저가 전체화면 요청을 처리하면서 화면 방향이나 viewport를 바꾸는 사례가 있어
  // 자동 전체화면 진입을 중단합니다. 게임은 실행 당시 캡처한 크기 그대로 유지합니다.
  lockInitialViewport120(document, { appShell: document.querySelector<HTMLElement>('#app') });
  if (forceToast) showToast('전체화면 전환 대신 실행 크기를 그대로 유지합니다.');
}

async function lockPortraitMode() {
  // Alpha 1.17: 방향 잠금/회전 요청 없음. 실행 당시 화면 크기를 viewportLock117이 유지합니다.
  lockInitialViewport120(document, { appShell: document.querySelector<HTMLElement>('#app') });
}
...
async function handleHealthAction085(action: string) {
  ...
  if (action === 'toggle-lite-mode') {
    liteRenderMode091 = false;
    try { localStorage.removeItem('soul-online-lite-render-091'); } catch { /* storage unavailable */ }
    syncSingleVisualMode117(document, { titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
    showToast('1.18부터 단일 표준 모드와 장면 고정 가드로 실행합니다.');
  }
  if (townContentOpen && (activeTownContent === 'settings' || activeTownContent === 'account')) renderTownContent();
  if (sheetOpen && activeSheetTab === 'account') renderSheet();
}
...
function renderSystemDoctor085(save: PlayerSave, mode: 'town' | 'account' | 'field') {
  ...
  const runtime112 = inspectMobileRuntime112(document, { root, titleScreen, loginScreen, townScreen, gameRoot: root });
  const fieldHud112 = inspectFieldHud112(document);
  const maintenance113 = inspectMaintenance113(document, { root, titleScreen, loginScreen, townScreen, gameRoot: root, sheet, townPanel: townContentPanel, itemDetailModal });
  const styleLoader115 = inspectStyleLoader115(document);
  const assetDelivery115 = inspectAssetDelivery115(document);
  const fieldLayout116 = inspectFieldLayout116(document, { titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  ...
  const runtime126 = inspectRuntimePolish126(document);
  ...
  const resilience133 = inspectResiliencePlan133(document, { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn }, saveService.getCloudWriteStatus());
  const release134 = inspectReleaseControl134(document, { appShell: document.querySelector<HTMLElement>('#app'), root, titleScreen, loginScreen, townScreen, gameRoot: root, startButton: startGameBtn });
  ...
  const rows: HealthTile087[] = [
    ...
    { label: '1.12 모바일 런타임', value: runtime112.message, level: runtime112.level, hint: runtime112.hint },
    { label: '1.12 HUD 세이프', value: fieldHud112.message, level: fieldHud112.level, hint: fieldHud112.hint },
    { label: '1.13 정리 레이어', value: maintenance113.message, level: maintenance113.level, hint: maintenance113.hint },
    { label: '1.15 CSS 로더', value: styleLoader115.message, level: styleLoader115.level, hint: styleLoader115.hint },
    { label: '1.15 에셋 배포', value: assetDelivery115.message, level: assetDelivery115.level, hint: assetDelivery115.hint },
    { label: '1.16 요청 UI', value: fieldLayout116.message, level: fieldLayout116.level, hint: fieldLayout116.hint },
    ...
    { label: '1.26 런타임 다듬기', value: runtime126.message, level: runtime126.level, hint: runtime126.hint },
    ...
    { label: '1.33 대안책/복구', value: resilience133.message, level: resilience133.level, hint: resilience133.hint },
    { label: '1.34 릴리즈/Actions', value: release134.message, level: release134.level, hint: release134.hint },
    ...
    { label: '화면 모드', value: '단일 표준', level: 'ok', hint: '1.17: 저화질/고화질 자동 전환 제거' }
  ];
